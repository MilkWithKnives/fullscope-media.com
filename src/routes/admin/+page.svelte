...
<Container class="py-12">
	<div class="max-w-7xl mx-auto">
		<div class="mb-8">
			...
			<p class="text-zinc-400">Manage your bookings and contact submissions</p>
		</div>
				...
				<div>
					...
					<div class="space-y-4">
						{#each bookings.slice(0, 10) as booking}
							<Card class="p-4 bg-zinc-900 border-zinc-800">
								<div class="flex justify-between items-start mb-3">
									<div>
										<h3 class="font-semibold text-white">{booking.client_name}</h3>
										<p class="text-sm text-zinc-400">{booking.service_type}</p>
									</div>
						...
						{#if bookings.length === 0}
							<Card class="p-8 text-center bg-zinc-900 border-zinc-800">
								<Calendar size={48} class="mx-auto text-zinc-600 mb-4" />
								<p class="text-zinc-400">No bookings yet</p>
							</Card>
						{/if}
					</div>
				</div>
					...
					<div class="space-y-4">
						{#each studioRentals.slice(0, 10) as rental}
							<Card class="p-4 bg-zinc-900 border-zinc-800">
								<div class="flex justify-between items-start mb-3">
									<div>
										<h3 class="font-semibold text-white">{rental.client_name}</h3>
										<p class="text-sm text-zinc-400 capitalize">{rental.rental_type} Rental - {rental.purpose}</p>
									</div>
						...
						{#if studioRentals.length === 0}
							<Card class="p-8 text-center bg-zinc-900 border-zinc-800">
								<Calendar size={48} class="mx-auto text-zinc-600 mb-4" />
								<p class="text-zinc-400">No studio rentals yet</p>
							</Card>
						{/if}
					</div>
				</div>
				...
				<div>
					...
					<div class="space-y-4">
							...
							<Card class="p-4 bg-zinc-900 border-zinc-800">
								<div class="flex justify-between items-start mb-3">
									<div>
										...
										<p class="text-sm text-zinc-400">{contact.service}</p>
									</div>
								...
								<p class="text-sm text-zinc-300 line-clamp-2">{contact.message}</p>
						...
						{#if contacts.length === 0}
							<Card class="p-8 text-center bg-zinc-900 border-zinc-800">
								<Mail size={48} class="mx-auto text-zinc-600 mb-4" />
								<p class="text-zinc-400">No contact submissions yet</p>
							</Card>
						{/if}
					</div>
				</div>
