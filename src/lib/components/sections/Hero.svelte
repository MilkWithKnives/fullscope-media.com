...
<section bind:this={heroRef} class="relative bg-black text-white overflow-hidden">
	...
	></div>
	<div
		class="absolute bottom-20 right-10 w-40 h-40 bg-[var(--color-primary-strong)]/15 rounded-full blur-2xl transition-transform duration-1000 ease-out"
		...
		<div class="py-24 lg:py-32">
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
				...
				<ScrollReveal animation="fade-left" delay={200} class="relative">
					<div class="aspect-video bg-gradient-to-br from-white/5 via-white/0 to-[var(--color-primary)]/10 rounded-2xl backdrop-blur-sm border border-white/10 flex items-center justify-center hover:scale-105 transition-transform duration-300 cursor-pointer group">
						<div class="text-center space-y-4">
							<div class="w-20 h-20 bg-white/10 rounded-full flex items-center justify-center mx-auto backdrop-blur-sm group-hover:bg-[var(--color-primary)]/30 transition-colors duration-300 group-hover:animate-bounce-slow">
								<Play size={32} class="text-white ml-1" />
							</div>
							<p class="text-gray-200 group-hover:text-white transition-colors duration-300">Watch Our Showreel</p>
						</div>
					</div>
