...
<div class="space-y-6">
	...
	{#if selectedDate}
		<Card class="p-6 bg-zinc-900 border-zinc-800">
			<h3 class="text-lg font-semibold text-white mb-4">Choose Your Package</h3>
			
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				{#each packages as pkg}
					<div class="border border-zinc-700 rounded-lg p-4 hover:border-[var(--color-primary)] transition-colors">
						<div class="flex items-center gap-3 mb-3">
							<pkg.icon class="text-[var(--color-primary)]" size={20} />
							<h4 class="font-semibold text-white">{pkg.name}</h4>
						</div>
						
						<p class="text-sm text-zinc-400 mb-3">{pkg.description}</p>
						
						<div class="flex items-center justify-between mb-4">
							<span class="text-2xl font-bold text-[var(--color-primary)]">${pkg.rate}</span>
							<span class="text-sm text-zinc-500">{pkg.minHours} hours min</span>
						</div>

						<!-- Time Slot Selection -->
						<div class="space-y-2">
							<p class="text-sm font-medium text-zinc-300">Available Times:</p>
