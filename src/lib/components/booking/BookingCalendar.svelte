...
<div class="space-y-6">
	...
	<Card>
		<div class="flex items-center gap-3 mb-4">
			...
			<h3 class="text-lg font-semibold text-gray-900">Select Service Type</h3>
		</div>
			...
			{#each serviceEntries as [key, service]}
				...
				>
					...
					<div class="text-sm">
						...
						{:else}
							<p class="font-medium text-blue-600">${service.rate_per_block}/15min</p>
						{/if}
						<p class="text-gray-500">Min: {formatDuration(service.minimum_blocks * 15)}</p>
						{#if service.setup_fee > 0}
							<p class="text-gray-500">Setup: ${service.setup_fee}</p>
						{/if}
					</div>
				</button>
			{/each}
		...
		<Card class="h-full">
			<div class="mb-4 flex items-center gap-3">
				...
				<div>
					...
					<p class="text-sm text-gray-600">Click on a date to see available time slots</p>
				</div>
			</div>
			...
			{#if selectedBooking}
				...
				<Card class="bg-blue-50 border-blue-200">
					<div class="flex items-center gap-3 mb-4">
						...
						<h3 class="text-lg font-semibold text-blue-900">Selected Booking</h3>
					</div>

					<div class="space-y-3 text-sm">
						<div class="flex justify-between">
							<span class="text-blue-700">Service:</span>
							<span class="font-medium text-blue-900">{service.name}</span>
						</div>
						<div class="flex justify-between">
							<span class="text-blue-700">Date:</span>
							<span class="font-medium text-blue-900">{new Date(selectedDate).toLocaleDateString()}</span>
						</div>
						<div class="flex justify-between">
							<span class="text-blue-700">Time:</span>
							<span class="font-medium text-blue-900">{formatTime(selectedBooking.start_time)} - {formatTime(selectedBooking.end_time)}</span>
						</div>
						<div class="flex justify-between">
							<span class="text-blue-700">Duration:</span>
							<span class="font-medium text-blue-900">{formatDuration(selectedBooking.duration_minutes)}</span>
						</div>
						{#if selectedBooking.total_amount > 0}
							<div class="border-t border-blue-200 pt-3">
								<div class="flex justify-between text-lg font-semibold">
									<span class="text-blue-900">Total:</span>
									<span class="text-blue-900">${selectedBooking.total_amount.toFixed(2)}</span>
								</div>
							</div>
						{:else}
							<div class="border-t border-blue-200 pt-3">
					...
					{:else}
						<Button class="w-full mt-4" onclick={() => onBookingSelect?.(selectedBooking!)}>
