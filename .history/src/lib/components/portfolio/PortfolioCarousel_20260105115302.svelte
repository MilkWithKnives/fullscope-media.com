					...
					<div class="mt-6 space-y-4">
						<div class="flex items-start justify-between">
							<div class="space-y-2">
								...
								<h3 class="text-2xl font-bold text-white group-hover:text-[var(--color-primary-strong)] transition-colors">
									{item.title}
								</h3>
						...
						<div class="flex flex-wrap gap-2">
							{#each item.tags as tag (tag)}
								<span class="px-3 py-1 bg-zinc-900 text-zinc-300 text-sm rounded-full border border-zinc-800 hover:bg-[var(--color-primary)]/10 hover:text-[var(--color-primary)] transition-colors">
									{tag}
								</span>
							{/each}
						</div>
