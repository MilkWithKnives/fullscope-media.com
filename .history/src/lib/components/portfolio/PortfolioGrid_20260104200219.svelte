...
{#snippet CardBody({ item }: { item: PortfolioItem })}
	...
	>
		...
		<!-- Content -->
		<div class="p-6 flex-1 flex flex-col bg-zinc-900 border-t border-zinc-800">
			<div class="space-y-3 flex-1">
				...
				<h3 class="text-lg font-bold text-white group-hover:text-[var(--color-primary-strong)] transition-colors line-clamp-2">
					{item.title}
				</h3>

				<p class="text-sm text-[var(--color-primary-strong)] font-medium">{item.client}</p>

				<p class="text-zinc-400 text-sm leading-relaxed line-clamp-3 flex-1">
					{item.description}
				</p>
			</div>
			...
			<div class="flex flex-wrap gap-1 mt-4">
				{#each item.tags.slice(0, 3) as tag (tag)}
					<span class="inline-flex items-center space-x-1 px-2 py-1 bg-zinc-800 text-zinc-300 text-xs rounded-full border border-zinc-700 hover:bg-[var(--color-primary)]/10 hover:text-[var(--color-primary)] transition-colors">
						<Tag size={8} />
						<span>{tag}</span>
					</span>
				{/each}
				{#if item.tags.length > 3}
					<span class="px-2 py-1 bg-zinc-800 text-zinc-300 text-xs rounded-full border border-zinc-700">
