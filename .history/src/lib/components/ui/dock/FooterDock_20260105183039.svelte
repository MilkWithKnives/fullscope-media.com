			...
			{#each navItems as item}
				<DockIcon {mouseX} {magnification} {distance}>
					<Tooltip content={item.label} sideOffset={8}>
						<a
							href={item.href}
							class="flex h-full w-full items-center justify-center rounded-full bg-zinc-800 text-zinc-400 transition-all duration-200 hover:bg-zinc-700 hover:text-white"
						>
