...
<Card class="max-w-2xl mx-auto bg-zinc-900 border-zinc-800">
	<div class="p-6">
		<h2 class="text-2xl font-bold text-white mb-6">Studio Rental Booking</h2>

		<!-- Booking Summary -->
		{#if selectedSlot}
			<div class="bg-zinc-800/50 rounded-lg p-4 mb-6 border border-zinc-700">
				<h3 class="font-semibold text-white mb-2">Rental Details</h3>
				<div class="grid grid-cols-2 gap-4 text-sm">
					<div>
						<span class="text-zinc-400">Date & Time:</span>
						<p class="text-zinc-200">{formatDateTime(selectedSlot.date, selectedSlot.start_time)}</p>
					</div>
					<div>
						<span class="text-zinc-400">Duration:</span>
						<p class="text-zinc-200">{Math.floor(selectedSlot.duration_minutes / 60)}h {selectedSlot.duration_minutes % 60}m ({selectedSlot.total_blocks} blocks)</p>
					</div>
					<div>
						<span class="text-zinc-400">Package:</span>
						<p class="text-zinc-200 capitalize">{selectedSlot.rental_type} Rental</p>
					</div>
					<div>
						<span class="text-zinc-400">Total:</span>
						<p class="text-[var(--color-primary)] font-bold text-lg">${selectedSlot.total_amount}</p>
					</div>
				</div>
			</div>
		{/if}
